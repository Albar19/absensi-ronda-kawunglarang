...
export default function NameSelector({ onSubmit, isSubmitting }: NameSelectorProps) {
  ...
  return (
    <div className="px-4 pb-8 space-y-5">
      {/* Instruction */}
      <div className="bg-[#eff6ff] border-2 border-[#1e3a8a] rounded-xl px-5 py-4">
        <p className="text-base font-bold text-[#1e3a8a] leading-snug">
      ...
      {/* Search + Dropdown */}
      <div>
        <label htmlFor="cari-nama" className="block text-base font-black text-slate-800 mb-2 tracking-wide uppercase">
          Pilih Nama Anda:
        </label>
        ...
        <div className="relative">
          ...
          <div className="relative flex items-center">
            <Search size={20} className="absolute left-4 text-slate-400 flex-shrink-0" strokeWidth={2.5} />
            <input
              id="cari-nama"
              type="text"
              inputMode="text"
              ...
              className="w-full pl-12 pr-4 py-4 text-lg font-semibold border-2 border-slate-400 rounded-xl bg-white text-slate-900 placeholder:text-slate-400 focus:border-[#1e3a8a] focus:outline-none transition-colors"
            ...
            <div className="absolute top-full left-0 right-0 bg-white border-2 border-[#1e3a8a] rounded-xl mt-1 shadow-lg z-50 overflow-hidden">
              {filteredWarga.map((warga) => (
                <button
                  ...
                  className="w-full text-left px-5 py-4 flex items-center justify-between border-b border-slate-100 last:border-0 hover:bg-blue-50 active:bg-blue-100 transition-colors"
                  style={{ minHeight: '60px' }}
                >
                  <div>
                    <p className="text-base font-bold text-slate-900 leading-tight">{warga.nama}</p>
                    <p className="text-sm text-slate-500 font-medium">{warga.rt}</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        {query.length > 0 && filteredWarga.length === 0 && (
          <p className="text-base text-slate-500 mt-2 px-1">
            Nama tidak ditemukan. Coba kata lain.
          </p>
        )}
        {query.length === 0 && (
          <p className="text-sm text-slate-400 mt-2 px-1">
      ...
      <button
        ...
        className={`w-full rounded-xl py-5 text-xl font-black text-white tracking-wide transition-all border-2 ${
          !selected
